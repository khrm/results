...
func (r *Reconciler) sendLog(ctx context.Context, o results.Object) error {
	...
	if !GVK.Empty() &&
		...
		isExists, err := r.resultsClient.IsLogRecordExists(ctx, o)
		if err != nil {
			return err
		}
		if isExists {
		...
		if err != nil {
			logger.Errorf("error creating log record: %v", err)
			return err
		}
		...
		logger.Infof("streaming logs for record '%s/%s'", o.GetNamespace(), o.GetName())

		go func() {
			err := r.streamLogs(ctx, o, logType, logName)
			if err != nil {
				logger.Errorf("error streaming logs for %s/%s: %v", o.GetNamespace(), o.GetName(), err)
			}
			logger.Infof("streaming completed '%s/%s'", o.GetNamespace(), o.GetName())
		}()
	}
