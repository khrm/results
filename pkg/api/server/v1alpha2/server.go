...
import (
	...
	"github.com/tektoncd/results/pkg/api/server/v1alpha2/plugin_logs"
...
type Server struct {
	...
	db              *gorm.DB
	auth            auth.Checker
	LogPluginServer *plugin_logs.LogPluginServer
...
func New(config *config.Config, logger *zap.SugaredLogger, db *gorm.DB, opts ...Option) (*Server, error) {
	...
	pluginServer, err := plugin_logs.New(srv.config, srv.logger, srv.auth, srv.db)
	if err != nil {
